...
slideShow.service("slides", function($http) {
  var slides;
  slides = [];
  $http.get("data/slides.json").success(function(data) {
    return data.forEach(function(slide) {
      return slides.push({
        active: false,
        title: slide.title,
        uri: slide.uri
...
slideShow.controller("slideCtrl", function($scope, slides) {
  $scope.slideSource = slides;
  $scope.activeSlide = 0;
  $scope.jumpTo = function(i) {
    if (i >= 0 && i < $scope.slideSource.length) {
      $scope.activeSlide = i;
  ...
  $scope.setActive = function(index) {
    $scope.slideSource.forEach(function(slide) {
      return slide.active = false;
    });
    if ($scope.slideSource[index]) {
      return $scope.slideSource[index].active = true;
    }
  };
  return $scope.refresh = function() {
    return $scope.setActive($scope.activeSlide);
  };
});
...
slideShow.directive("slide", function() {
  return {
    restrict: 'E',
    scope: {
      slideContent: "="
